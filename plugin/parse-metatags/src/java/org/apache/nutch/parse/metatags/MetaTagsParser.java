  ...
  public Parse filter(String url, WebPage page, Parse parse,
    ...
    while (iterator.hasNext()) {
      ...
      if (metatagset.contains("*") || metatagset.contains(mdName.toLowerCase())) {
        ...
        LOG.debug("Found meta tag: '" + mdName + "', with value: '" + value
            + "'");
        metadata.put(PARSE_META_PREFIX + mdName.toLowerCase(),
                ByteBuffer.wrap(value.getBytes()));
      }
    }
    Iterator<Entry<String, ByteBuffer>> itm = metadata.entrySet().iterator();
    while (iterator.hasNext()) {
      Entry<String, ByteBuffer> entry = itm.next();
    ...
    while (it.hasNext()) {
      ...
      if (metatagset.contains("*") || metatagset.contains(name.toLowerCase())) {
        ...
        page.getMetadata().put(PARSE_META_PREFIX + name.toLowerCase(),
